...
public:
   ...
   * be equal to that of the returned vector of pairs. Nevetheless, both
   ...
   * called a second time with another type because it filters (deletes)
   * the names of those on the first call which used a different type.
   * Thus, the 'types' argument is for the case where variables of
   * multiple types are allowed to pass through.
   *
   ...
   * variables of this type in the system, if any) or a std::unique_ptr to
